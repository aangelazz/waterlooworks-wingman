...
## Fix pass 1 — post-review hardening

A reviewer pass (REVIEW.md) found one P0 and four P1s, all scale/robustness rather than logic bugs. Fixes, each validated with Node harnesses in `/tmp/ww-fix` (same staged-TS-copy technique as M2):

- **P0 — `agent.ts` analyze was one giant call** that truncated mid-JSON-array past ~70 postings (Gemini's 8,192 output-token cap) and blew Groq's free-tier TPM. Now batches 25 postings per `chatJSON` call, merges the analyses, and runs the quantile `retier()` **once** over the full set (per-batch tiering would have skewed the distribution — harness confirms 6/15/24/15 S/A/B/C at n=60). If a mid-batch call fails, completed batches are kept and surfaced via a new `PartialAnalysisError`; App shows the partial tier list *and* the error. The positional-fallback id matching had to become per-batch, which the harness also covers.
- **P1 — no fetch timeout in `llm.ts`:** a hung connection wedged "Parsing…"/"Ranking…" forever. Added `AbortSignal.timeout(60s)` to the POST, mapping `TimeoutError`/`AbortError` to a readable `LLMError`.
- **P1 — `embeddings.ts` cached a rejected model-download promise forever**, so a failed 25 MB download bricked the Map tab until reload. A `.catch` on the chain now resets `extractorPromise = null` before rethrowing, so re-opening the tab genuinely retries. (WebGPU→WASM fallback untouched; also deleted the unused `embeddingsReady()` export the reviewer flagged.)
- **P1 — `parse.ts` multi-chunk extraction was all-or-nothing:** one 429 mid-loop discarded every already-parsed chunk. A mid-loop failure now throws `PartialExtractionError` carrying the parsed postings + failed-chunk count; `PasteImport` keeps the partial results, leaves the paste text intact, and shows "Parsed N, but M of K chunks failed…".
- **P1 — one-shot embedding of all postings** meant a multi-second main-thread freeze (and ~hundreds of MB of transient attention buffers) at n=100 on WASM, redone on every Map visit. `embed()` now runs mini-batches of 12 with an event-loop yield between them, and a new `embedPostings()` caches vectors module-level keyed by `id + text` — tab revisits and preference reblends only embed what changed. Harness verifies batch slicing alignment and cache hit/miss/bust behavior.
- **Approved P2s:** (a) `SettingsModal` now validates the key at save time with a tiny live `chatJSON` ping — ✓ verified / "key saved, but the test call failed: …" feedback in the modal; (b) `mergeById` dedupes within the incoming batch too, so a paste with repeated rows can't create duplicates; (c) the stale-`simScores`-closure race in `handleRank` is gone — sim scores moved from state to a `useRef`, re-read after the analyze await and folded in via `reblend`, so embeddings finishing mid-rank are no longer silently dropped.

**Surprise of the pass:** `tsconfig` has `erasableSyntaxOnly: true`, so the new error classes can't use TS constructor parameter properties — plain field declarations + assignment instead. Everything else was as sketched in the review. `npm run build` clean (zero TS errors, main chunk ~248 kB).

## Design pass — dark reskin, presentation only

Rebuilt the look without touching behavior: a warm dark-only palette (near-black `#0f0d0b` base, warm card surfaces, off-white ink, 1px borders instead of shadows) with a single warm-amber accent, all defined as Tailwind v4 `@theme` tokens in `index.css` (`--color-bg/surface/edge/ink/accent…`, so no scattered hex). Typography moved off the system stack to self-hosted Fontsource variable fonts — **Fraunces** (editorial serif) for the wordmark, tab headers, section headings and tier letters, **Instrument Sans** for body/UI — bundled as local woff2 so the "no third-party runtime calls" promise holds (no font CDN). De-emoji'd the chrome: goose and gear gone, odds badges are now a colored dot + text label, flags use dot markers, the modal close is an inline SVG, and the favicon is a serif "W" monogram on a dark rounded square. Tabs became an underline row, tier rows got serif letters with a hairline rule, and the similarity map was recolored for dark (muted-but-separable cluster hues, subtle grid, dark-styled tooltip). Status/tier/odds colors stay distinguishable but sit at the 300-weight/10%-tint range so nothing glows. `npm run build` clean; the two `@fontsource-variable` packages are the only new dependencies.
